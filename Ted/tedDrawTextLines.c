...
int tedDrawSpan(	const DrawTextLine *		dtl,
    ...
    docDrawSetColorNumber( dc, (void *)sdd, ta->taTextColorNumber);//colorcount);//ta->taTextColorNumber

    /* colorcount = (colorcount + 1) % 4; */

    DocumentRectangle ibar;
    int tempx0 = lc->lcOx;
    ...
    if(!mdstring){
      ...
      memcpy(mdstring, printString, nbLen);
    }
	...
	tedDrawSegments( dc, spanX0- lc->lcOx, y- lc->lcOy,
					...
					mdstring, segments, segmentCount );
	}
    else{
	drawString( lc->lcDrawingSurface,
				spanX0- lc->lcOx, y- lc->lcOy,
					fullScreenFont, mdstring, nbLen );
	}
    dc->dcLayoutContext.lcOx = tempx0;
    /*
